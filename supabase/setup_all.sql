-- ============================================
-- MJ Health Companion - Complete Database Setup
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all access' AND tablename = 'medications') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all access' AND tablename = 'timeline_events') THEN
    CREATE POLICY "Allow all access" ON timeline_events FOR ALL USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all access' AND tablename = 'appointments') THEN
    CREATE POLICY "Allow all access" ON appointments FOR ALL USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all access' AND tablename = 'lab_results') THEN
...
-- 4. SEED DATA - Current Medications
